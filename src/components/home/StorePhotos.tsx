...
export function StorePhotos() {
  return (
    <ul className="no-scrollbar -mx-4 flex snap-x gap-3 overflow-x-auto px-4 scroll-pl-4 sm:-mx-6 sm:px-6 sm:scroll-pl-6 lg:mx-0 lg:grid lg:grid-cols-5 lg:px-0">
      {photos.map((p, i) => (
        <li key={p.src} className={i === 0 ? 'w-40 shrink-0 snap-start lg:w-auto' : 'w-56 shrink-0 snap-start lg:w-auto'}>
          <img
            src={p.src}
            alt={p.alt}
            loading="lazy"
            decoding="async"
            className={i === 0 ? 'aspect-[3/4] w-full rounded-2xl object-cover shadow-card' : 'aspect-[4/3] w-full rounded-2xl object-cover shadow-card lg:aspect-[3/4]'}
          />
        </li>
      ))}
    </ul>
